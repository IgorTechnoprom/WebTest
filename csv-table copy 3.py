...
with open('list.html', 'r') as html_file:
    ...
    for item in modifier_items:
        keyword = item.find(class_='modifier-card-title alpha__title').text.strip()
        modifier_suggestion = item.find(class_='modifier-suggestion').text.strip()
        ...
        if keyword in existing_data:
            if (modifier_suggestion != existing_data[keyword]['Modifiers'] or
                    search_vol != existing_data[keyword]['Search Vol'] or
                    cpc != existing_data[keyword]['CPC']):
                # Update CSV data
                existing_data[keyword]['Modifiers'] = modifier_suggestion
